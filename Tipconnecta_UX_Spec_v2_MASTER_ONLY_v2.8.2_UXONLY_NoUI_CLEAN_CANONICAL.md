...
> Kontext: EPIC 4 **rozšiřuje** (a místy „přepisuje“) broker marketplace obrazovky z EPIC 2 v oblastech: platnost tiketu, matching, LTV.
...
## 3) Rozhodnutí (výchozí nastavení pro MVP)

Tato sekce nahrazuje původní „Nejasnosti“ a obsahuje výchozí rozhodnutí pro MVP. Pokud se v budoucnu některé body změní, upravíme je zde a v navazujících kapitolách (UX + data).

### 3.1 Pool program
- **MVP:** Pool program není v UI aktivně podporovaný (neonboardingujeme, nekomunikujeme benefit).
- **Datově:** počítáme s příznakem pro budoucí rozšíření (zapnuto/vypnuto + případně typ/úroveň).
- **Admin:** může pool program pro brokera zapnout/vypnout (konfiguračně); v MVP má dopad pouze informační.

### 3.2 Founding Partners
- **MVP:** Founding Partner = **označení (badge)** u brokera (profil + admin seznam brokerů).
- **MVP benefit:** výchozí navýšení limitu paralelních aktivních rezervací (slotů) o **+1** oproti standardu (admin může individuálně přepsat).
- **Marketing:** program je interní (nepoužíváme jako veřejný claim).

### 3.3 Schvalování brokera (admin)
- Stavový model: `Pending review` → `Approved` / `Rejected` / `Needs info`.
- Admin ověřuje minimálně:
  - identitu a kontakty (jméno, e-mail, telefon),
  - IČO / registraci firmy (pokud relevantní),
  - smluvní a AML dokumenty, které jsou pro onboarding definované,
  - bankovní účet pro výplatu provizí,
  - souhlas s podmínkami platformy.
- `Needs info`: admin pošle brokerovi konkrétní požadavek; broker doplní údaje a znovu odešle ke schválení.

### 3.4 Více uživatelů na účtu developera
- **MVP:** 1 uživatel = 1 developer účet.
- **Post‑MVP:** více uživatelů s rolemi `Owner` / `Member` (minimálně pro finance a právní).

### 3.5 Agentura / zástupce
- **MVP:** samostatná role agentury/zástupce není podporována.
- Pokud broker vystupuje jako agentura, je veden jako „Broker účet“ a odpovědnost (provize, komunikace) zůstává na brokerovi.

### 3.6 Spolupráce více brokerů na jednom tiketu
- **MVP:** každý tiket má jednoho vlastníka (brokera).
- Sdílení provize mezi brokery v MVP neřešíme v aplikaci (pouze mimo platformu).

### 3.7 Chování při expiraci / zrušení rezervace
- Pokud investor nedokončí podpis v SLA, rezervace se automaticky ukončí jako **expirovaná** a tiket se vrátí do nabídky (s audit záznamem důvodu).
- Broker může rezervaci zrušit ručně kdykoli před aktivací (vyžadujeme důvod); investorovi se odešle notifikace e‑mailem (broker má i in‑app notifikaci).
- Developer může rezervaci ukončit / zamítnout dle stavů procesu (vyžadujeme důvod).

### 3.8 Podpisové balíčky (eSign)
- **MVP: 2 podpisové kroky / 2 balíčky:**
  1) `Souhlas se sdílením údajů + NDA` (investor ↔ platforma) – po podpisu se odemkne identita developera.
  2) `Rezervační smlouva` (investor ↔ developer) – po podpisu se rezervace může aktivovat dle procesu.
- Autentizace podpisu: výchozí **e-mail link**; volitelně **SMS OTP** (nastavitelné adminem na úrovni tiketu nebo rezervace).
- Upomínky: automaticky **24h před deadlinem** + při expiraci (konfigurovatelné).

### 3.9 Support / řešení sporů
- Support kanál je konfigurovatelný (admin nastavuje e-mail/URL); v MVP primárně e‑mail.
- Spory mezi stranami jsou v MVP řešeny mimo platformu; v aplikaci evidujeme pouze interní admin poznámku k rezervaci/tiketu.
...
> Kontext: datový dokument uvádí obecný model provizí a vypořádání (včetně toho, že platí developer).
...
Tato kapitola nahrazuje původní „Nejasnosti“ u eSign procesu konkrétními pravidly.

- **SLA / expirace:** pokud investor nepodepíše v definované lhůtě, rezervace automaticky expiruje, uloží se důvod a tiket se vrátí do nabídky.
- **Remindery:** systém posílá upomínku 24h před expirací + finální informaci při expiraci.
- **Kontrasign (platforma / developer):**
  - `Souhlas + NDA`: v MVP evidujeme podpis investora jako rozhodující (platforma může používat před‑podepsaný template / automatický podpis).
  - `Rezervační smlouva`: druhá strana (developer) je vedená jako signatář; po podpisu investora je developer vyzván k podpisu (nebo potvrzení podle nastavení).
- **OTP:** výchozí autentizace = e‑mail link; SMS OTP je volitelná politika (admin přepínač na úrovni tiketu/rezervace).
- **Rušení rezervace:** broker může zrušit před aktivací; důvod je povinný a ukládá se do auditu.
...
**Pravidlo: ** „Zahájit rezervaci“ probíhá jako **dvoukrokový modal**: (1) výběr/založení investora, (2) odeslání eSign balíku.
...
> Kontext: V MVP používáme **1 investor SLA** 48h pro celý balík. Stav 3 se používá jen pro informování a auditní logiku.
...
- Sekce „Identita & přístup“ (karta):  
  ...
  - **Broker:** {vy}  
  - Pravidlo: **Identita se odemyká po podpisu Souhlasu+NDA.**  
...
## Rozhodnutí (DPH v MVP)

- **DPH režim není „odhad“:** v datech je explicitně uložený a UI jej pouze zobrazuje.
- **Platforma:** má globální nastavení `platform_is_vat_payer` (admin konfiguruje dle právní reality).
- **Developer:** má atribut `developer_is_vat_payer` (+ DIČ / země, pokud relevantní).
- **Provizní případ / faktura:** obsahuje uložené výpočtové hodnoty (`net`, `vat`, `gross`) a odkaz na použitý režim.
- **UI pravidlo:** částky v UI nepřepočítáváme „za běhu“, ale vždy bereme uložené hodnoty.
- **Výchozí nastavení pro nového developera:** `developer_is_vat_payer = false`, dokud to developer v profilu nepotvrdí.
...
> Kontext: Dokumenty jsou navázané na projekt (ne na tiket). Tiket odkazuje na projekt a bere z něj obrázky/dokumenty (s řízenou viditelností).
...
- Kontext: faktické „smazání“ může být v praxi anonymizace + retenční výjimky (audit musí zůstat).
...
- Sekce „Moji investoři — kvalita dat“:
  ...
  - Stat: **2 investoři bez preferencí**
...
- Sekce „Připravenost tiketů“:
  ...
  - Řádek: `T‑Draft‑03 · Chybí: dokumenty (2), LTV (volitelně)`
...
- Sekce „Provize (defaulty)“
  ...
  - Pravidlo: *Per tiket lze nastavit individuálně při schválení tiketu.*
...
> Kontext: Názvy jsou návrh. Důležitý je **rozsah** a **payload**, ne přesná URL.
