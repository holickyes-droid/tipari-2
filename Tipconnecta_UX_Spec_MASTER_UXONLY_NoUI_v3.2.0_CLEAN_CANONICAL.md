...
**Doc ID:** TIPCONNECTA-UX-MASTER-UXONLY-3.2.0
**Verze:** v3.2.0 (UX ONLY)
...
- Tipconnecta_Souhrn_Dat_DATA_FINAL_CLEAN_v1.4.1_CANONICAL.md (data model + pravidla)
- Tipconnecta_Sitemap_Broker_v1.3.1_CLEAN_CANONICAL.md
- Tipconnecta_Sitemap_Developer_v1.3.0_CLEAN_CANONICAL.md
- Tipconnecta_Sitemap_Admin_v1.3.0_CLEAN_CANONICAL.md
- Tipconnecta_Brand_Manual_v2.4.0_InvestownStyle_MASTER_CLEAN_CANONICAL.md (jen pro tón/brand – UI řeší design systém)
...
- **Hlavní menu je vždy nahoře (top bar)**: vlevo logo → Dashboard, uprostřed hlavní sekce podle role, vpravo uživatelské menu (profil, nastavení, odhlásit). Nepoužívat persistentní levý sidebar.
...
**Interakce:**
- Přepínač zobrazení: **Seznam (1 karta/řádek)** ↔ **Mřížka (3 karty vedle sebe)**; pamatuje si poslední volbu.
...
- Seznam „payoutů“ (provize + případné pool bonusy) jako položky se stavem: **Připravuje se → Čeká na fakturu → Ověření → Schváleno → Odesláno → Zaplaceno**.
- Každá payout položka obsahuje: období, související tiket(y)/rezervace, částku, měnu, DPH režim, splatnost a **předmět fakturace** (zadává admin; broker ho vidí a může ho použít na faktuře).
- Broker u payoutu:
  - stáhne podklady (rekap + instrukce),
  - **nahraje fakturu** (soubor, číslo faktury, datum vystavení, splatnost),
  - vidí historii změn a poznámku od admina.
- **Pool program fakturace (rozhodnutí):**
  - Pool bonus se v MVP vede jako **samostatná payout položka** (kvůli účetnímu oddělení),
  - admin může (později) povolit sloučení do jedné faktury – nastavitelné v admin settings.

### 5.9 Pool (broker)
- Cíl: motivovat brokery k vyššímu objemu investic a kvalitě spolupráce (bonusový program).
- Zobrazeno brokerovi:
  - Aktuální status v programu (aktivní/neaktivní) + pravidla (shrnutí).
  - „Můj výkon“ (objem investic, počet aktivních rezervací, potvrzené investice, vyplacené provize).
  - **Moje anonymizované pořadí**: např. „Jsi v TOP 15 %“ nebo „Pořadí #12 z 184“ – **bez zobrazení identity ostatních**.
  - Distribuce (graf/škála) jen jako anonymní kontext (percentily), zvýrazněná pozice uživatele.
- Pravidla:
  - Nikdy nezobrazovat jména/firmy jiných brokerů.
  - Pořadí se přepočítává periodicky (např. denně).
